...
function BodyButtons(props: BodyButtonsType) {
    ...
    return(
        <td>
            {props.isArchive ? "" : <input onClick={() => showEditForm(props.id)} 
                type="image" src={getButtonIcon("edit")}/>}

            {props.isArchive 
            ?<input onClick={() => {unarchivNote(props.id)}} type="image" src={getButtonIcon("archiv")}/>            
            :<input onClick={() => {archivNote(props.id)}} type="image" src={getButtonIcon("archiv")}/>}
                
            {props.isArchive ? "" : <input onClick={() => deleteNote(props.id) }
                 type="image" src={getButtonIcon("delete")}/>}
        </td>
    )
}
