...
class SendFormContainer extends React.Component<ISendFormContainer & IDispath> {
  ...
  getId() {
    ...
    if (id === '0') {
      const lastNoteId = this.props.notesTable.body[this.props.notesTable.body.length - 1].id; //
      id = 1 + +lastNoteId;
    }
  ...
  getCurrentData() {
    ...
    const month = this.parsNumberToMonthName(data.getMonth() + 1); // getMonth() return 0-11 so +1
  ...
  sendForm = (contentName: string, category: string, content: string) => {
    ...
    this.props.notesTable.editForm == 0 ? this.props.addNote(note) : this.props.sendEditNote(note, this.id);
  };

  render(): JSX.Element {
    return (
      <SendForm
        noteLine={this.getEditLine()}
        currentData={this.getCurrentData()}
        selectArray={this.props.notesTable.selectArray}
