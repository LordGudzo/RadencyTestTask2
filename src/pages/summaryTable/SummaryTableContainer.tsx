...
class SummaryTableContainer extends React.Component<Object & SummaryTableType> {
  ...
  summaryCount() {
    const dataStore = this.props.noteTable.body;
    const archivStore = this.props.noteTable.archivedNotes;

    this.props.noteTable.selectArray.map((category) => {
      const activAmount = this.countCategory(category, dataStore);
      const archivAmount = this.countCategory(category, archivStore);
